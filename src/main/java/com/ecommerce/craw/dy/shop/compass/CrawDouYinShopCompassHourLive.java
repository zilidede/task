...
//爬取抖音直播榜单
public class CrawDouYinShopCompassHourLive extends CrawBaseDouYinList {

    private Integer hour = 0;
    private  int crawCount = 0;
    private final int maxCrawCount = 1000;
    private  Long minSaleSums=10*30l;
    public CrawDouYinShopCompassHourLive() throws Exception {
        ...
        setXhrList(Ini4jUtils.traSpecificSection("douYinShopApi"));
    }
        ...
        @Override
        public void save(){
            LoggerUtils.logger.debug("解析xhr,并导入到数据库");
        ...
        public void selectTime(String time,String timeType) throws InterruptedException {
            ...
            if (timeType.indexOf("小时榜") >= 0) {
                time=String.valueOf(getHour())+":00";
            }
            //选择时间；
            String xpath = "//*[@class=\"ecom-radio-group ecom-radio-group-outline\"]/label";
            List<ChromiumElement> elements = getTab().eles(By.xpath(xpath)); //时间类型选择框
            Thread.sleep(1000);
            int len = elements.size();
            if (len < 1) {
            ...
            if (len == 5) {
                if (timeType.equals("小时榜")) {
                    ...
                    ChromiumElement elements1 = getTab().ele(By.xpath(xpath)); //小时选择框
