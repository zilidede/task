...
public class CrawTrendinsightKeywords extends CrawServiceXImpl {
    ...
    private final Integer maxCrawCount = 10; // 最大爬取次数
    ...
    public CrawTrendinsightKeywords() throws Exception {
        ...
        srcDir = Ini4jUtils.readIni("xhrDir");
    ...
    public List<HttpVO> craw(String keyword, String location, String startTime, String endTime) throws InterruptedException {
        //爬取单一关键词
        ...
        if (locationTag) {
            if (location.equals("")) {
                elements.get(0).click().click(); //打开关键字热度
                Thread.sleep(1000 * 2);
                if (!startTime.equals(""))
                    selectTime(startTime, endTime);//选择时间差；
                xpath = "//*[@class=\"byted-select-input-content-wrapper\"]";
                ...
                Thread.sleep(1000 * 2);
                List<ChromiumElement> element1 = elements2.get(0).eles(By.xpath("./div"));//获取区域一级列表
                Boolean flag1 = true;
                //一级分类
                for (int j = 0; j < element1.size(); j++) {
                    String name = element1.get(j).text();
                    ...
                    if (name.equals("江西")) {
                        ...
                        Thread.sleep(1000 * 2);
                        element1 = elements2.get(0).eles(By.xpath("./div"));//获取区域一级列表
                        Thread.sleep(1000);
                        j = 6;
                        continue;
                    }
                    //   * 爬取区域二级类目
                    element1.get(j).click().click(); //选择一级类目
                    Thread.sleep(1000 * 2);
                    xpath = "//*[@class=\"ReactVirtualized__Grid__innerScrollContainer\"]";
                    List<ChromiumElement> elementsLocation = getTab().eles(By.xpath(xpath));
                    if (elementsLocation.size() == 1) {
                        //无二级类目
                        ...
                        for (int k = 0; k < element3.size(); k++) {
                            //重新建立选择框
                            if (k == 15 && flag2) {
                                element3 = elementsLocation.get(1).eles(By.xpath("./div"));//获取区域二级列表
                                k = 6;
                                continue;
                            }
                            selectSecondLocation(element3.get(k));
                            element1  = getTab().eles(By.xpath("//*[@class=\"ReactVirtualized__Grid__innerScrollContainer\"]")).get(0).eles(By.xpath("./div")); // 获取区域选择框内容框列表个数
                            Thread.sleep(1000 * 2);
    ...
    public void selectSecondLocation(ChromiumElement element) throws InterruptedException {
        ChromiumElement selectElement = element.ele(By.xpath("./label"));
        getTab().actions().moveTo(selectElement);
        Thread.sleep(1000 * 3);
        selectElement.click().click();
        Thread.sleep(1000 * 3);
        selectElement = element.ele(By.xpath("./label"));
        Thread.sleep(1000 * 3);
        selectElement.click().click();
        Thread.sleep(1000 * 3);
        Thread.sleep(1000 * 1);
        ...
        if(crawCount > maxCrawCount){
            LoggerUtils.logger.warn("翻页次数超过最大限制，保存xhr");
            saveXhr();
        }
    }
