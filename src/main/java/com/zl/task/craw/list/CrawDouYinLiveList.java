...
//爬取抖音直播榜单
public class CrawDouYinLiveList extends CrawBaseDouYinList {
    CrawDouYinLiveList() throws Exception {
    ...
    public void craw(TaskVO task) throws Exception {
        ...
        String s11 = elements.get(1).text();
        elements = getTab().eles(By.xpath(xpath)).get(1).eles(By.xpath("./li"));
        Integer ilen = Integer.parseInt(strings[1]);
        elements.get(ilen).click().click(); //行业类目选择-选择行业；
        Thread.sleep(1000);
//*[@id="garfish_app_for_compass-shop-chance_gn951kek"]/div/div[2]/div[2]/div/form/div[2]/div[1]/div/div/div[2]/div/div/div/div/div[1]
        // 爬取自营和合作
        xpath = "//*[@class=\"ecom-form-item-control-input-content\"]";
        List<ChromiumElement> elements11 = getTab().eles(By.xpath(xpath)).get(0).eles(By.xpath("./div[1]/div[1]/div"));
        for (int i = 0; i < elements11.size(); i++) {
            System.out.println(elements11.get(i).text());
            elements11.get(i).click();
            Thread.sleep(1000);
